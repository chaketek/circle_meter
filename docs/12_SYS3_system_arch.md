...
| Buzzer | G3 | |
| Power HOLD | G46 | 電源保持 |
| **PORT.A (I2C)** | G13 / G15 | **CAN TX/RX に割当**（`OPN-02` 解決済み。G13 = TX, G15 = RX） |
| **PORT.B (GPIO)** | G1 / G2 | 未使用（将来の拡張用に確保） |

### 3.1 CAN 配線設計

ESP32-S3 の TWAI コントローラは GPIO マトリクス経由で任意の GPIO に割り当て可能であるため、
I2C 用に用意されている **PORT.A (G13/G15)** をそのまま CAN に使用する。

| Grove 線色 | CAN Unit 側 | M5Dial PORT.A | ファームウェア設定 |
|---|---|---|---|
| 黒 | GND | GND | — |
...
**`OPN-02` は 2026-09-21 のブリングアップで解決した。** 上記の組み合わせで
PCAN から送出したフレームを受信できることを実測で確認している。
TX/RX を入れ替えると受信数が 0 のままになるため、配線を触らずに切り分けられる。
