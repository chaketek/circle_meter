...
| `SYS-07` | **CAN バスへフレームを送出しないこと**（データフレーム・リモートフレームのいずれも）。ただし CAN プロトコルが要求する **ACK は返すこと**。ACK を返さないと送信側が再送を繰り返し、最終的にバスオフに陥るため（`RSK-10`） | STK-13, CST-04 | `IT-04` |
...
| `SYS-18` | 電源投入時にオープニング画面を表示すること。表示時間は **3.0 秒**を上限とし、その間も CAN 受信は開始していること | STK-08 | `QT-06` |
...
| `RSK-06` | 誤ってバスへフレームを送出し、ECU の CAN 処理に影響する | **高** | 低 | 3 層で防ぐ: ① 送信 API を実装しない ② 送信キュー長を 0 にして物理的にキューイングできなくする ③ CI の静的チェックで `twai_transmit` の混入を検出する | `SYS-07`, `IT-04` |
...
| `RSK-10` | **本機が ACK を返さないことで rusEFI が送信を完了できず、再送を繰り返してバスオフに陥る** | **高**（ECU の CAN が停止する。`RSK-06` を避けるつもりの対策が逆に ECU を壊す） | **高**（バス上が rusEFI と本機の 2 ノードだけなら必ず発生。NA ロードスターは純正 CAN を持たないためこの構成になりやすい） | TWAI を NORMAL モードで初期化し、受信したフレームに ACK を返す。ACK はプロトコル上必須の参加であり「書き込み」ではない | `SYS-07`, `IT-04`, `IT-15` |
...
| `OPN-07` | ESP32-S3 の TWAI を Grove ポートの GPIO に割り当てられるか | **可能**。GPIO マトリクス経由で任意の GPIO に割り当てられる | 2026-09-21 |
| `OPN-02` | CAN Unit をどのポートに繋ぎ、TX/RX はどの GPIO か | **PORT.A に接続。`TX = G13` / `RX = G15`**。PCAN から送出したフレームの受信を実機で確認。PORT.B (G1/G2) 設定のままでは `rx=0` かつ `rec=0`（RX ピンに信号が来ない状態）になる | 2026-09-21 |
| `OPN-08` | Listen Only モードで 2 ノード構成のバスが成立するか | **成立しない**。ACK を返さないため送信側が再送を繰り返す。実測で受信レートが 120 f/s の想定に対し 3650 f/s に達し、先頭フレームが再送に張り付いて後続フレームがバスに出られなくなった。`RSK-10` として登録し、NORMAL モードへ変更（`DEC-04`） | 2026-09-21 |
