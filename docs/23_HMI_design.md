...
| ロゴ形式 | 240 x 240 以内の PNG を LVGL の C 配列（`LV_COLOR_FORMAT_RGB565A8`）に変換し `assets/logo.c` として保持 |
| アニメーション | 0.0–0.4 s フェードイン → 0.4–1.2 s 保持 → 1.2–1.5 s フェードアウト |
| 短縮条件 | CAN フレームを受信したら 0.4 s 経過後に即座に次画面へ遷移してよい |
| 差し替え | `tools/make_logo.py` で PNG → C 配列に変換。素材は `assets/logo_src.png` | `OPN-05` |
