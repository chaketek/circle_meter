...
| フォント | **円に収まる最大サイズを実行時に自動選択する**（§3.4）。実測値: AFR `14.7` = **76 px**、λ `1.000` = **62 px** |
...
その高さで使える幅は `2 * sqrt(r^2 - dy^2)` になる。
固定サイズを決め打ちすると、桁数が変わる λ (`1.000` = 5 文字) と
AFR (`14.7` = 4 文字) のどちらかが必ず小さすぎるか、はみ出す。

そこで **実行時に候補を大きい順に試し、円に収まる最初のものを採用する**。
候補には小数倍のスケールを混ぜる。整数倍だけだと AFR で 1.5 倍程度しか取れないため。

| 表示 | 実測サイズ | 従来比 |
|---|---|---|
| AFR `14.7` | 76 px 高 / 152 px 幅 | 2.4 倍 |
| λ `1.000` | 62 px 高 / 159 px 幅 | 1.9 倍 |
| EGT `845C` | 41 px 高 / 94 px 幅 | 2.6 倍 |

各要素がぶつからないよう、縦方向のバンドごとに高さ上限を与える。
...
> λ が AFR より小さくなるのは文字数が 1 つ多いためである。
> 同じ大きさに揃えたい場合は λ を小数 2 桁にする（`SWR-65` の設定項目にする候補）。
...
| アニメーション | 0.0–0.8 s フェードイン → 1.6 s 保持 → 0.6 s フェードアウト（合計 3.0 s）。**バックライト PWM のランプで実現**する（画素のアルファ合成は 16 bit 階調では縞が出るうえ CPU を食うため） |
