...
#### 実測（P1 ブリングアップ FW、2026-09-21）

LVGL を使わない素朴な実装（毎フレーム全画面クリア → 図形・文字描画 → 240×240 全面を
1 回で転送）で測定した結果:
...
| 空きヒープ | 237 KB（全画面スプライト 112.5 KB 確保後、変動なし） |

最も非効率な描き方でも要求（30 fps = 33 ms）の約半分の時間で収まっている。
LVGL の部分描画（`DEC-05`）を入れればさらに短縮されるため、`SYS-12` は達成可能と判断する。
ただしこれは λ リングと数値のみの画面であり、`QT-03` で本番 UI を対象に再測定する。

> **補足**: この計測は「直接ディスプレイに描くと全消去と再描画の間が見えて明滅する」
> 問題への対処としてオフスクリーンバッファを導入した際に得られたもの。
> 明滅は LVGL 導入後は部分描画により原理的に発生しない。
...
1. 書き込み中は**完了を待ってから**読み出す（書き込み側は数十ナノ秒で抜けるため停滞しない）
2. 万一全試行が失敗した場合は、空のスナップショットではなく
   **直前に成功したスナップショット**を返す。`takenAtMs` は現在時刻にするので
   鮮度は正しく老い、本当に途絶していれば `Stale` → `Lost` に落ちる（`RSK-01` は損なわれない）

回帰テストは `UT-15`。修正前のコードでは 20 万回の読み出し中 **132,746 回 (66 %)** が
誤って `Lost` を返す。
